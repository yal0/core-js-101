...
function isLeapYear(date) {
  ...
  return Boolean(
    (!(year % 4) && year % 100) // год делится на 4 кроме годов кратным 100
    || !(year % 400), //  они должны делится ещё и на 400
  );
}
...
function angleBetweenClockHands(date) {
  ...
  let degree = Math.abs(30 * hours + date.getUTCMinutes() / 2 - date.getUTCMinutes() * 6);
