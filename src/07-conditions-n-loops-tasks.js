...
function isCreditCardNumber(ccn) {
  ...
  let n = str.slice(0, str.length - 1).split('').reverse() // массив цифр без последней и разворачиваем, т. к. цифры удваиваются начиная с правой
    .map((item, index) => { // удваиваем цифры через одну,
      // если получилось больше 10 складываем цифры
      if ((index + 1) % 2) {
        if (item < 5) return 2 * item;
        return Math.floor((2 * item) / 10) + 2 * item - 10;
      }
      return +item;
    })
    .reduce((sum, item) => sum + item, 0);
  n = (10 - (n % 10)) % 10;
  return n === Number(str[str.length - 1]);
}
...
function getDigitalRoot(num) {
  return num > 9 ? getDigitalRoot(num.toString().split('').reduce((sum, number) => sum + Number(number), 0)) : num;
}
...
function isBracketsBalanced(str) {
  ...
  const brascets = [];
  for (let i = 0; i <= str.length; i += 1) {
    if (open.includes(str[i])) { // открывающая скобка
      brascets.push(str[i]);
    } else if (close.includes(str[i])) { // закрывающая скобка
      if (brascets.pop() !== open[close.indexOf(str[i])]) {
        return false; // нет подходящей открывающей скобки
      }
    }
  }
  return !brascets.length; // число открытых и закрытых скобок сошлось
}
...
function getCommonDirectoryPath(pathes) {
  let result = '';
  const minStrLength = pathes
    .reduce((min, current) => ((!min || current.length < min) ? current.length : min), 0);
  for (let j = 0; j < minStrLength; j += 1) {
